...
const Cart = () => {
    const {products, addToCart, updateCartItem, removeFromCart, cartItems, getCartCount, getCartAmount, navigate} = useContextProvider();
    const dummyaddress = "Nepal"
  ...
  return products.length > 0 && cartItems ? (
    ...
    <div className="bg-gray-100 px-4 md:px-12 lg:px-16 xl:px-16 py-4">
        ...
        <div className="rounded-lg md:w-2/3">
            <div className="flex justify-between items-centerfont-medium p-3
            bg-[#4CB944]">
            ...
            return (
                <div key={index} className="justify-between mb-6 rounded-lg bg-white p-3 shadow-md sm:flex sm:justify-start bg-gray-500">
                    <div className="flex w-1/2 gap-4">
                        ...
                        <p className="mt-1 text-xs text-gray-700">500g</p>
                        </div>
                    </div>
                        <div className="mt-4 w-1/2 flex justify-between items-center gap-6">
                            <p className="mt-1 text-gray-700">${product.offerPrice}</p>
                            <div className="flex items-center border-gray-100">
                                <span className="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50" onClick={() => {removeFromCart(product.id)}}> - </span>
                                <input className="h-8 w-8 border bg-white text-center text-xs outline-none" value={product.quantity} min="1" readOnly />
                                <span className="cursor-pointer rounded-r bg-gray-100 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50" onClick={() => {addToCart(product.id)}}> + </span>
                            </div>
                            <div className="flex items-center space-x-4">
                                <p className="text-sm">${(product.quantity * product.offerPrice).toFixed(2)}</p>
        ...
        <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
            ...
            <div className="">
                <p className="mb-1 text-lg font-bold">$vvvvvvvcvvvv{getCartAmount()}</p>
                <p className="text-sm text-gray-700">including VAT</p>
            </div>
            </div>
            <button className="mt-6 w-full rounded-md bg-[#4CB944] py-1.5 font-medium text-blue-50 hover:bg-[#417B38]">Check out</button>
        </div>
        </div>
    </div>
    </>
